...
PATH = r'C:\VSCODE\pyqt\music_playlist\tracks.json'
...
class TrackWindow(QWidget):
    ...
    def delete_track(self):
        ...
        reply = QMessageBox.question(self, 'Подтверждение', f'Удалить трек "{self.current_track}"?', 
                                     QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
        
        if reply == QMessageBox.StandardButton.Yes:
            if self.service.delete_track(self.current_track):
                QMessageBox.information(self, "Успех", f"Трек '{self.current_track}' удален")
                self.current_track = None
                self.refresh_list()
    ...
    def edit_track(self):
        ...
        def save_changes():
            ...
            if self.service.update_track(self.current_track, new_name, new_genre, new_author, new_year):
                QMessageBox.information(dialog, "Успех", f"Трек обновлен")
                dialog.close()
                self.refresh_list()
            else:
